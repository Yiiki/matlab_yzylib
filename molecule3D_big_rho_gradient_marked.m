function axout = molecule3D_big_rho_gradient_marked(varargin)
%   set except atomic number
%   other atoms are hidden deeply
%   update from molecule3D_big_rho_gradient
%   by YZY
%   mapping a gradient FaceAlpha to the object
%   update from molecule3D_big_rho.m
%   by YZY
...
DBOND = 1.7; % maximal distance of atoms forming bond

% resolution of spheres and cylinders
NS = 20; % spheres, more looks smoother
NB = 20; % more looks smoother
...
% which is optimized for the large data set
% and the periodicity is not used for dist
pairs_tmp=zeros(0.5*Na*(Na-1),2);
ds_tmp=zeros(0.5*Na*(Na-1),1);
itmp=0;
max_bond=3;% unit: Angstrom
for i=1:Na-1
    if(strcmp(lab{i},'Au')) % Au is excluded for bonding
        continue
    end
for j=i+1:Na
    if(strcmp(lab{j},'Au'))
    ...
    for itype=1:size(bond_menu,1)
        ...
        ilogi= ...
         (strcmp(lab(i),atom1) & strcmp(lab(j),atom2)) ...
        |(strcmp(lab(i),atom2) & strcmp(lab(j),atom1)) ...
        |(strcmp(atom1,'YYY1') & strcmp(atom2,'YYY2'));
        % fprintf('itype=%d th bond, bond(itype)=%.6f\n',itype,str2double(bond_menu(itype,3)))
        if(ilogi)
        bond_tmp=str2double(bond_menu(itype,3));
        if(dr<bond_tmp)
            itmp=itmp+1;
            pairs_tmp(itmp,1)=i;
            pairs_tmp(itmp,2)=j;
...
pairs=pairs_tmp(1:itmp,1:2);
ds=ds_tmp(1:itmp);

% % here is the old verison,
% % which could be slow on the large data set
...
for k = 1:size(xyz,1)
    ...
    alpha=func_alpha(xyz(k,2),func_excep(lab{k}));
    thiscol = 1-(1-col(lab{k})).*alpha;
...
for k = ks % draw sticks for all bounds
    ...
    % redefine r1 and r2 to move them onto the surface of ball
    r1=r1+rad(lab{pairs(k,1)}).*v;
    r2=r2-rad(lab{pairs(k,2)}).*v;
    ...
    cyl2 = bd/2; % length half bond cylinder, rad(lab{pairs(k,1)}) + bd/2
    cyl1 = bd; % length full bond cylinder
    ...
    alpha=func_alpha2(max(r1(2),r2(2)),max(func_excep(lab{pairs(k,1)}),func_excep(lab{pairs(k,2)})));
    
    % get colors of both atoms
    thiscol1 = 1-(1-col(lab{pairs(k,2)})).*alpha;
    thiscol2 = 1-(1-col(lab{pairs(k,1)})).*alpha;
...
%     case  'Si', c = [0.5 0.5 0.6];51,51,255
    case  'Si', c = [0.9 0.5 1.0];
    ...
    case   'Au', c=[248, 212, 128]./256;
    case   'Ga', c= [ 0,128,255]./256;
    case   'As', c= [116,208,87]./256;
    otherwise, c = [44,117,255]./256;
end
...
% ----------------------------------------
% alpha function
function alp=func_alpha(y,sft)
alp=exp(-((y+sft)/12).^4);
function alp=func_alpha2(y,sft)
alp=exp(-((y+sft)/12).^4);
function r=func_excep(s)
switch s
    case  'Bk', r = 0;
    otherwise, r = 4;
end